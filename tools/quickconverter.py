...
#main function
def main():
    #ask user if they'd like to convert Units or CSGs/Features
    print("---Welcome to papa quickconverter---")
    print("-Are you converting units? yes = 1 -")
    isConvertingUnits = int(input())
    if isConvertingUnits == 1:
        fbxCommand = "papatran --texture-mode strip -o"
    else:
        fbxCommand = "papatran.exe --texture-mode reference -o"
    ...
    #find and remove QuickCode and papadump from the list
    ...
    for file in foundFiles:
        ...
        if file.find('.fbx') != -1:
            ...
            command = f'{fbxCommand} {fileShort}.papa {fileShort}.fbx'
            os.system(command)
            #check for success
            for root, dirs, files in os.walk(directory):
